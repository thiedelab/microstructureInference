...
from microstructure_inference.dataModules import cubic_proper_point_group_operations, digitize_radial_distance, digitize_polarAngle, digitize_braggIntensity
from microstructure_inference.LossFunctions import pointGroup_map_rotation_prediction, pointGroup_map_rotation_prediction_return_geodesic_distance_stack, pointGroup_map_rotation_and_phase_prediction, symmetric_orthogonalization
...
def detect_Bragg_Disks_in_4DSTEM_data_normalize_intensity(aligned_data,
                                      crystal_unit_cell_file_path = "./",
                                      correlation_thresh_for_templ_match = 15000,
                                      correlation_thresh_for_direct_beam = 8e4,
                                      diffGauss_sigma1 = 2, 
                                      diffGauss_sigma2 = 6,                                      
                                      collection_pixel_nums = 6,
    ...
    K = shifted_correlation_kernel - np.mean(shifted_correlation_kernel)    
    customPROBE = K / np.linalg.norm(K)
    ...
    for si in range(real_scan_space_x_size):
        for sj in range(real_scan_space_y_size):
            # c_qx = np.copy(bragg_peaks.raw[si, sj].data['qx'])
            # c_qy = np.copy(bragg_peaks.raw[si, sj].data['qy'])
            # c_qx_qy = np.stack([c_qx, c_qy]).T
            
            # c_intensity = mean_intensity_within_circle_custom_axes(aligned_data[si,sj], c_qx_qy)
            bragg_peaks.raw[si,sj].data['intensity'] = bragg_peaks.raw[si,sj].data['intensity'] / np.max(bragg_peaks.raw[si,sj].data['intensity'])
