from orientationMapping.LossFunctions import binry_map_objective_criterion
import torch
import numpy as np
...
def train_epoch(model, dataloader, optimizer, device, PAD = 0):
    ...
    for idx, (x, y, z)  in  pbar:
        ...
        # print("pred", pred)
        loss, rotatLoss, mirrorLoss, predicted_rotation_matrix = binry_map_objective_criterion(pred, labels_r, labels_m)
        loss = loss.to(device)
