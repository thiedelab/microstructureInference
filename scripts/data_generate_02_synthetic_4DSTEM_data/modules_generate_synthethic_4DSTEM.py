...
def generate_synthetic_diffraction_pattern(sim_diffrac_patt_img , 
                                           ...
                                           random_seed = 42,
                                           sim_BD_intensity_scaling_factor_mean = 0.05, 
                                           sim_BD_intensity_scaling_factor_std = 0.01,
                                           sim_BD_minimum_intensity_threshold = 1e-9,
                                           conv_img_intensity_scaling_factor_mean = 1.0,
    ...
    translated_convResult = warp(convResult, tform_simBD.inverse)

    # fig, ax = plt.subplots(1, 1, figsize=(6, 6))
    # ax.set_title("translated_convResult")
    # im = ax.imshow(translated_convResult,  norm = 'log', cmap = 'gray')
    # ax.set_xlabel(r"$q_y$", fontsize = 16)
    # ax.set_ylabel(r"$q_x$", fontsize = 16)
    # plt.colorbar(im, ax = ax, fraction=0.0468, pad=0.02)
    # plt.show()
    ...
    translated_convResult_intensity_max_x, translated_convResult_intensity_max_y = np.unravel_index(np.argmax(translated_convResult), translated_convResult.shape)
    
    # print("translated_convResult_intensity_max_x, translated_convResult_intensity_max_y", translated_convResult_intensity_max_x, translated_convResult_intensity_max_y)
    
    translated_convResult_center_x, translated_convResult_center_y = int(translated_convResult.shape[0] / 2), int(translated_convResult.shape[1] / 2)
    translated_convResult_after_removing_direct_beam = np.copy(translated_convResult)
    translated_convResult_after_removing_direct_beam[int(translated_convResult_center_x - simulated_direct_beam_removal_range):int(translated_convResult_center_x + simulated_direct_beam_removal_range), int(translated_convResult_center_y - simulated_direct_beam_removal_range):int(translated_convResult_center_y + simulated_direct_beam_removal_range)] = 0.0


    trans_conv_Img_scaling_factor = np.random.normal(conv_img_intensity_scaling_factor_mean, conv_img_intensity_scaling_factor_std, translated_convResult_after_removing_direct_beam.shape)
    ...
    translated_convResult_after_removing_direct_beam = translated_convResult_after_removing_direct_beam * trans_conv_Img_scaling_factor
    if normalized_bgs_avg_for_masking_sim_img is not None:
        translated_convResult_after_removing_direct_beam = translated_convResult_after_removing_direct_beam * normalized_bgs_avg_for_masking_sim_img 

    translated_convResult_after_removing_direct_beam = skimage.filters.gaussian(translated_convResult_after_removing_direct_beam, sigma=gaussian_blur_sigma, preserve_range=True)

    

    # fig, ax = plt.subplots(1, 1, figsize=(6, 6))
    # ax.set_title("translated_convResult_after_removing_direct_beam")
    # im = ax.imshow(translated_convResult_after_removing_direct_beam,  norm = 'log', cmap = 'gray')
    # ax.set_xlabel(r"$q_y$", fontsize = 16)
    # ax.set_ylabel(r"$q_x$", fontsize = 16)
    # plt.colorbar(im, ax = ax, fraction=0.0468, pad=0.02)
    # plt.show()
    ...
    synthetic_diffraction_pattern = translated_BG * background_scaling_factor + translated_convResult_after_removing_direct_beam
