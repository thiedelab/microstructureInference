...
EXEC_SCRIPT="${BASE_DIR}/computeTime/orientation_map_via_py4DSTEM.py"
INPUT_FILE_PATH="${BASE_DIR}/sampled_and_stacked/fourD_braggVectors_pointLists/"
OUTPUT_FILE_PATH="${BASE_DIR}/computeTime/output_SD${SCAN_DIM}_ZADP${ZA_DP}_ID${STARTID}-${ENDID}_py4DSTEM/"
RESULT_DIR="${BASE_DIR}/computeTime/timing_log_SD${SCAN_DIM}_ZADP${ZA_DP}_ID${STARTID}-${ENDID}_py4DSTEM"

mkdir -p "$OUTPUT_FILE_PATH" "$RESULT_DIR"

BENCH_LOG="${RESULT_DIR}/summary.txt"

# ... (Metadata logging remains the same for CPU/GPU info) ...

echo -e "\n# Run_ID\tElapsed(s)\tUser_CPU(s)\tSys_CPU(s)" > "$BENCH_LOG"
...
for ((i=STARTID; i<STARTID+N_RUNS; i++)); do
    WALL_TIME_FILE="${RESULT_DIR}/timing_SD${SCAN_DIM}_ZADP${ZA_DP}_ID${i}.tmp"
    PYTHON_OUTPUT_FILE="${RESULT_DIR}/output_SD${SCAN_DIM}_ZADP${ZA_DP}_ID${i}.log"
    ...
    /usr/bin/time -f "${i}\t%e\t%U\t%S" \
        ...
        python "$EXEC_SCRIPT" \
            ...
            --run_id "$i" \
            --scan_dim "$SCAN_DIM" \
            --zone_axis_displacement "$ZA_DP" > "$PYTHON_OUTPUT_FILE" 2>&1
